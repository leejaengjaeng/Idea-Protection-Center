    ...
    <c:import url="/WEB-INF/views/import/admin_header.jsp" />
        <article>
            ...
            <table class="manage_table" style="margin-bottom:20px; ">
                ...
                <c:forEach items="${designAdminList }" var="list" varStatus="status">
	                <tr>
	                	<input type="hidden" value="${list.getDeid() }"/>
	                    <td>${list.getUserID()}</td>
	                    <td >${list.getUserName()}</td>
						
	                    <!-- pName 없으면 선택하게 버튼나오도록 -->
	                    <c:choose>
	                    	<c:when test="${list.getPatentName()==null}">
	                    		<td><div id="p${status.count}"></div><button id="btn${status.count}" onclick="giveLawyer('${list.getDeid()}','btn${status.count}','p${status.count}')">지정</button></td>
	                    	</c:when>
	                    	<c:otherwise>
	                    		<td>${list.getPatentName()}</td>
	                    	</c:otherwise>
	                    </c:choose>	                   
	                    <td>${list.getUserEmail()}</td>
	                    <td>${list.getTitle()}</td>
	                    <td>${list.getD_condition()}</td>
